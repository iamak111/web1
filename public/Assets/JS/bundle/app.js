...
const add_to_carts = document.querySelectorAll(".add_to_carts");
...
window.addEventListener("load", (event) => {
  ...
  if (main_img_link && main_img_src) {
    [...document.querySelectorAll(".sld_gallry")].map((el) => {
      el.addEventListener("click", (e) => {
        main_img_link.setAttribute("href", e.target.dataset.img);
        main_img_src.src = e.target.dataset.img;
      });
    });
  }
});
...
if (vendor_verification) {
  vendor_verification.addEventListener("click", (e) => {
    ...
    if (e.target.dataset.status === "accepted") {
      return updateVendorStatus("accepted", vendor_verification.dataset.id);
    } else
      return updateVendorStatus("rejected", vendor_verification.dataset.id);
  });
}
