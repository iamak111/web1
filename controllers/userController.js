...
exports.checkProductADo = catchAsync(async (req, res, next) => {
  ...
  if (!req.body.size) req.body.size = req.body.data.selectedProductSize || null;
  if (cart) {
...
exports.addressForOrder = catchAsync(async (req, res, next) => {
  ...
  const address = await addressModel.deleteMany({
    userId: req.user._id,
    ecmaeId: { $ne: uid },
  });
